...
bool Odbc::DeleteData(std::string tableName, std::wstring id, std::wstring pw)
{
	if (!_IsConnect || _Inst == nullptr)
		return false;

	std::wstring query = L"DELETE FROM ";
	query += utf8_to_wstring(tableName);
	query += L" WHERE UserID = ";
	query += id;
	query += L" AND PassWord = ";
	query += pw;
...
bool Odbc::CheckDataExist(std::string tableName, std::string colName, std::wstring value)
{
	if (!_IsConnect || _Inst == nullptr)
		return false;
...
bool Odbc::AddUser(std::wstring id, std::wstring password)
{
	/*if(isUserExist(id, password))
		return false;

	if (!InsertData("Users", "UserID, passWord"," '" +  wstring_to_utf8(id) + "', '" + wstring_to_utf8(password) + "' "))
		return false;
		return true;
		*/
	if (!_IsConnect || _Inst == nullptr)
	...
	std::wstring query = L"CALL AddUser( '";
	query += id;
	query += L"', '";
	query += password;
	query += L"')";
...
bool Odbc::CheckLogin(std::wstring id, std::wstring password)
{
	if (isUserExist(id, password))
		return true;
...
bool Odbc::DeleteUser(std::wstring id, std::wstring password)
{
	if (!_IsConnect || _Inst == nullptr)
		return false;

	std::wstring query = L"CALL DeleteUser( '";
	query += id;
	query += L"', '";
	query += password;
	query += L"')";
	...
	if (!DeleteData("Users",utf8_to_wstring(" '") + id + utf8_to_wstring("' "), utf8_to_wstring(" '") + password + utf8_to_wstring("' ")))
		return false;
...
bool Odbc::isUserExist(std::wstring id, std::wstring pw)
{
	if (CheckDataExist("Users", "UserID, PassWord", "( '" + this->wstring_to_utf8(id) + "' , '" + this->wstring_to_utf8(pw) + "' )" ))
		return true;
	else
		return false;
}
