...
namespace NLogicLib
{
	...
	ERROR_CODE PacketProcess::LobbySecretChat(PacketInfo packetInfo)
	{


		CHECK_START
		auto reqPkt = (NCommon::PktLobbySecretChatReq*)packetInfo.pRefData;
		NCommon::PktLobbySecretChatRes resPkt;

		auto pUserRet = m_pRefUserMgr->GetUser(packetInfo.SessionIndex);
		auto errorCode = std::get<0>(pUserRet);
		auto toUser = m_pRefUserMgr->FindUser(reqPkt->UserID);
		...
		if (toUser == nullptr)
		{
			CHECK_ERROR(ERROR_CODE::LOBBY_SECRET_CHAT_INVALID_SEND_USER);
		}
		...
		auto pUser = std::get<1>(pUserRet);
		//같은 로비인지 체크
		if (toUser->GetLobbyIndex() != pUser->GetLobbyIndex())
		{
			CHECK_ERROR(ERROR_CODE::LOBBY_USER_DIFFERENT_DOMAIN);
		}
		...
		pLobby->SecretChat(pUser->GetSessioIndex(), pUser->GetID().c_str(), reqPkt->Msg, toUser->GetIndex());
		//m_pRefNetwork->SendData(toUser->GetIndex(), )
		m_pRefNetwork->SendData(packetInfo.SessionIndex, (short)PACKET_ID::LOBBY_SECRET_CHAT_RES, sizeof(resPkt), (char*)&resPkt);
		return ERROR_CODE::NONE;
